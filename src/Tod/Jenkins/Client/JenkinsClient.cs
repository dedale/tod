...
internal sealed class JenkinsClient(JenkinsConfig config, string userName, string userToken, IApiClient? apiClient = null) : IJenkinsClient, IDisposable
{
    ...
    {
        var url = $"{config.Url}/{jobName.UrlPath}/api/json?tree=builds[id,number,result,timestamp,duration,building,changeSets[items[commitId,msg,author[fullName],authorEmail]]]{{0,{count}}}";
