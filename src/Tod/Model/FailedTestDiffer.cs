...
internal static class FailedTestDiffer
{
        ...
        IFlakyTests flakyTests)
    {
        ...
        var (i, j) = (0, 0);
        while (i < sortedBaseline.Length && j < sortedOnDemand.Length)
        {
            var baseline = sortedBaseline[i];
            var onDemand = sortedOnDemand[j];
            var comparison = string.Compare(baseline.Key, onDemand.Key, StringComparison.Ordinal);
            if (comparison == 0)
            {
                // Common
                if (!baseline.FailedTest.Equals(onDemand.FailedTest))
                {
                    // Updated
                    testResults.Add(new FailedTestResult(onDemand.FailedTest, Newness.Updated, flakyTests.IsFlaky(baselineJob, onDemand.FailedTest)));
            ...
            {
                // OnDemand only
                testResults.Add(new FailedTestResult(onDemand.FailedTest, Newness.New, flakyTests.IsFlaky(baselineJob, onDemand.FailedTest)));
                status |= TestBuildDiffStatus.NewFailures;
                j++;
            }
        }
        // Handle remaining items in onDemand that come after all baseline items
        while (j < sortedOnDemand.Length)
        {
            testResults.Add(new FailedTestResult(sortedOnDemand[j].FailedTest, Newness.New, flakyTests.IsFlaky(baselineJob, sortedOnDemand[j].FailedTest)));
            status |= TestBuildDiffStatus.NewFailures;
            j++;
        }
